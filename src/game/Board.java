...
public class Board {
    ...
    public void addRandomTile() {
        int t = StdRandom.uniform(openSpaces.size());
        BoardSpot spot = openSpaces.get(t);
        double randomValue = StdRandom.uniform();
        int v = randomValue < 0.1 ? 4 : 2;
        int x = StdRandom.uniform(0, 1); // HOW TO DO THE PART WHEN V < 0.1 TILE = 4 AND OTHERWISE 2
        gameBoard[spot.getRow()][spot.getCol()] = v;
    ...
    public void transpose() {
        for (int i = 0; i < gameBoard.length; i++) {
            for (int j = i + 1; j < gameBoard[i].length; j++) {
